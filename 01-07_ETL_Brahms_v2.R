...
#load parameters
source(paste0(thisdir,"/parameters_toBrahms_v2.R"))
setwd(thisdir)
...
for (source in RICOVERI_OSPEDALIERI_table){
  ...
  CLINICAL_ITEMS<-CLINICAL_ITEMS[clin_item_datatype==1 | clin_item_datatype==2,item_classification:='40'][clin_item_datatype==4, item_classification:='40']
  ...
  CLINICAL_ITEMS<-CLINICAL_ITEMS[,person_id:=substr(person_id, 4,24)]
  CLINICAL_ITEMS<-CLINICAL_ITEMS[str_detect(person_id,"^ANA"), person_id:=substr(person_id, 4,24)]
  ...
  ENCOUNTERS<-ENCOUNTERS[,person_id:=substr(person_id, 4,24)]
  ENCOUNTERS<-ENCOUNTERS[str_detect(person_id,"^ANA"), person_id:=substr(person_id, 4,24)]
...
for (source in SPECIALISTICA_table){
  ...
  pippo<-pippo[,encounter_id:=1:.N]
  pippo<-pippo[,dataprest:=as.Date(dataprest)]
...
DRUG_ITEMS<-DRUG_ITEMS[,person_id:=substr(person_id, 4,24)]
DRUG_ITEMS<-DRUG_ITEMS[str_detect(person_id,"^ANA"), person_id:=substr(person_id, 4,24)]
#DRUG_ITEMS<-DRUG_ITEMS[,person_id:=as.numeric(person_id)]
                         
DRUG_ITEMS<-DRUG_ITEMS[,`:=`(drug_item_src_id="",
                             encounter_id="",
                             item_classification=1,
                             drug_item_datatype=3,
                             ...
                             drug_item_adm_route="",
                             drug_item_dose_form="")] 
#drug_item_setting=‘5’ if from community pharmacy (irrespective of which organization purchased the drug) 
                  #‘2’ if from hospital pharmacy
DRUG_ITEMS<-DRUG_ITEMS[origine=="fed",drug_item_setting:=2]
DRUG_ITEMS<-DRUG_ITEMS[origine=="spf",drug_item_setting:=5]
...
# transform right fotmat for person_id
PERSONS<-PERSONS[,person_id:=substr(id, 4,24)]
PERSONS<-PERSONS[str_detect(person_id,"^ANA"), person_id:=substr(person_id, 4,24)]
...
OBSERVATION_PERIODS<-OBSERVATION_PERIODS[,.(id,data_inizioass,data_fineass,source)]

## ADD run of CreateSpell function
...
setkeyv(
  ...
  c("id", "entry_spell_category", "exit_spell_category", "num_spell", "source")
)
...
#renamed vars
setnames(output_spells_category,"id","person_id")
setnames(output_spells_category,"entry_spell_category","obs_period_start_date")
setnames(output_spells_category,"exit_spell_category","obs_period_end_date")
output_spells_category<-output_spells_category[,obs_period_end_date:=min(obs_period_end_date, end_study, na.rm = T),by=c("person_id","num_spell")]
output_spells_category<-output_spells_category[,obs_period_id:=1:.N]

OBSERVATION_PERIODS<-merge(output_spells_category,PERSONS[,.(person_id,death_date)],by="person_id",all.x = T)
...
# adapt person_id to shorter integer
recode_PERSONS<-PERSONS_all[,.(person_id)]
recode_PERSONS[,new_person_id:=as.numeric(paste0(20220303,1:.N))]
fwrite(recode_PERSONS,paste0(dirtemp,"/recode_PERSONS.csv"), quote = "auto")

PERSONS_all<-merge(PERSONS_all, recode_PERSONS, by="person_id")
...
# adapt person_id to shorter integer
OBSERVATION_PERIODS_all<-merge(OBSERVATION_PERIODS_all, recode_PERSONS, by="person_id", all.x = T)
...
# adapt person_id to shorter integer
ENCOUNTERS_all<-merge(ENCOUNTERS_all, recode_PERSONS, by="person_id", all.x = T)
ENCOUNTERS_all<-ENCOUNTERS_all[,-"person_id"]
setnames(ENCOUNTERS_all,"new_person_id","person_id")
ENCOUNTERS_all<-ENCOUNTERS_all[,.(encounter_id, encounter_src_id, person_id,encounter_setting,encounter_start_date, encounter_end_date, encounter_derm_specialty,encounter_specialty, encounter_discharge_status,encounter_misc)] 
...
#check whether clin_item_date is missing
CLINICAL_ITEMS_all<-CLINICAL_ITEMS_all[!is.na(clin_item_date),]
# adapt person_id to shorter integer
CLINICAL_ITEMS_all<-merge(CLINICAL_ITEMS_all, recode_PERSONS, by="person_id", all.x = T)
CLINICAL_ITEMS_all<-CLINICAL_ITEMS_all[,-"person_id"]
setnames(CLINICAL_ITEMS_all,"new_person_id","person_id")
CLINICAL_ITEMS_all<-CLINICAL_ITEMS_all[,.(clin_item_id,clin_item_src_id,person_id,encounter_id,clin_item_code,item_classification,clin_item_datatype,clin_item_setting,clin_item_date)]
#check whether person_id are all in PERSONS
...
#check whether drug_item_code is missing
DRUG_ITEMS_all<-DRUG_ITEMS_all[!is.na(drug_item_code) & drug_item_code!="" & drug_item_code!="0",]
# adapt person_id to shorter integer
DRUG_ITEMS_all<-merge(DRUG_ITEMS_all, recode_PERSONS, by="person_id", all.x = T)
DRUG_ITEMS_all<-DRUG_ITEMS_all[,-"person_id"]
setnames(DRUG_ITEMS_all,"new_person_id","person_id")
DRUG_ITEMS_all<-DRUG_ITEMS_all[,.(drug_item_id,drug_item_src_id,person_id,encounter_id,drug_item_code,item_classification,drug_item_datatype,drug_item_setting,drug_item_npc,drug_item_presc_specialty,drug_item_date,drug_item_dosage_available,drug_item_quantity,drug_item_quantity_unit,drug_item_total_ddd,drug_item_presc_daily_dose,drug_item_days_supply,drug_item_strength,drug_item_strength_unit,drug_item_adm_route,drug_item_dose_form)] 
#check whether person_id are all in PERSONS
